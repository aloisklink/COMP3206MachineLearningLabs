...
2000 Samples were generated, and the first 1500 samples were used as a training set, and the last 500 as a testing set. The test data set can be found in Figure \ref{fig:timeSeriesLinearRegression}. \(P\), the number of previous samples to be used as the input, was set to \(P = 20\). Because of this, the first 20 samples could not be used as an input, as they lacked the necessary previous samples. The following code details a MATLAB function that implements this:

\lstinputlisting[language=Matlab]{convertTimeSeriesToMLInput.m}
